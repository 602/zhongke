...
- (id)initWithFrame:(CGRect)frame index:(NSInteger)index title:(NSString *)title norColor:(UIColor *)norColor selColor:(UIColor *)selColor isSelected:(BOOL)isSelected {
    ...
    if (self) {
         ...
         *  设置文字
         */
        NSArray *strArr = [title componentsSeparatedByString:@"\n"];
        NSRange attrRange = [title rangeOfString:strArr[1]];
...
- (void)selectAtIndex:(NSInteger)index {
    if (self.itemsArray &&
        ...
        UIView *itemView = [self.itemsArray objectAtIndex:index];
        
        [itemView touchesBegan:[NSSet new] withEvent:[UIEvent new]];
    }
}
